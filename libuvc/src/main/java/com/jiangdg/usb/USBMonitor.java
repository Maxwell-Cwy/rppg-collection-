...
public final class USBMonitor {
			...
			if (context != null) {
				if (Build.VERSION.SDK_INT >= 31) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                        flags |= PendingIntent.FLAG_IMMUTABLE; // 或者如果是隐式 Intent 必须用 IMMUTABLE
                    }
    ...
    public synchronized void register() throws IllegalStateException {
        ...
        if (mPermissionIntent == null) {
            ...
            final Context context = mWeakContext.get();
            if (context != null) {
                // --- 核心修复 1: 确定 PendingIntent 标志位 ---
                int flags = PendingIntent.FLAG_UPDATE_CURRENT;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    flags |= PendingIntent.FLAG_IMMUTABLE;
                }

                // --- 核心修复 2: 显式 Intent (针对 Android 14 安全加强) ---
                Intent permissionIntent = new Intent(ACTION_USB_PERMISSION);
                // 非常关键：显式指定包名，防止 Android 14 报隐式 Intent 安全异常
                permissionIntent.setPackage(context.getPackageName());

                mPermissionIntent = PendingIntent.getBroadcast(context, 0, permissionIntent, flags);

                // --- 核心修复 3: 广播接收器导出标志 (针对 Android 14) ---
                final IntentFilter filter = new IntentFilter(ACTION_USB_PERMISSION);
                filter.addAction(ACTION_USB_DEVICE_ATTACHED);
                filter.addAction(UsbManager.ACTION_USB_DEVICE_DETACHED);

                if (Build.VERSION.SDK_INT >= 34) {
                    // Android 14 必须明确指定 RECEIVER_EXPORTED 或 RECEIVER_NOT_EXPORTED
                    // 系统广播（如 USB）通常需要 EXPORTED，或者至少让它能接收系统发出的意图
